...
class BiLSTM_CRF(nn.Module):
    ...
    def _get_lstm_features(self, sentence):
        self.hidden = self.init_hidden()
        embeds = self.word_embeds(sentence).view(sentence.size(0), 1, -1)
        lstm_out, self.hidden = self.lstm(embeds, self.hidden)
        lstm_out = lstm_out.view(sentence.size(0), self.hidden_dim)
    ...
    def _score_sentence(self, feats, tags):
        ...
        tags = torch.cat([torch.tensor([self.tag_to_ix['<sos>']], dtype=torch.long).cuda().view(1, 1), tags])
